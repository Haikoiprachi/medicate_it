...
const AGENTS = [
  {
    ...
    icon: "",
    color: "#00d4ff",
    desc: "Parsing & structuring symptoms",
  },
  {
    id: "analyzer",
    name: "Risk Analyzer",
    icon: "",
    color: "#ff6b35",
    desc: "Identifying conditions & red flags",
  },
  {
    id: "scorer",
    name: "Risk Scorer",
    icon: "",
    color: "#a855f7",
    desc: "Calculating dynamic risk score",
  },
  {
    id: "triage",
    ...
    icon: "",
    color: "#22c55e",
    desc: "Determining urgency & care pathway",
  },
];
...
const URGENCY_CONFIG = {
  EMERGENCY: {
    ...
    text: "white",
    label: "EMERGENCY",
    sub: "Call 112 immediately",
  },
  URGENT: {
    bg: "#ff6d00",
    text: "white",
    label: "URGENT",
    sub: "Go to ER within 1–2 hours",
  },
  SEMI_URGENT: {
    bg: "#ffd600",
    text: "#1a1a1a",
    label: "SEMI-URGENT",
    sub: "See a doctor within 24 hours",
  },
  NON_URGENT: {
    bg: "#00c853",
    text: "white",
    label: "NON-URGENT",
    sub: "Schedule a doctor appointment",
  },
  SELF_CARE: {
    bg: "#2979ff",
    text: "white",
    label: "SELF-CARE",
    sub: "Home care with monitoring",
  },
};
...
const AgentCard = ({ agent, status }) => {
  ...
  return (
    <div
      style={{
        background: "#0f172a",
        border: `1.5px solid ${borders[status]}`,
        borderRadius: 10,
        padding: "10px 14px",
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ color: "#334155", fontSize: 13, marginTop: 1 }}>
            {agent.desc}
          </div>
        </div>
        <div style={{ fontFamily: "monospace", fontSize: 13, flexShrink: 0 }}>
          ...
          {status === "running" && (
            <span
              style={{ color: agent.color, animation: "blink 0.8s infinite" }}
            >
              ● RUN
            </span>
          )}
      ...
      <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 4 }}>
        <div
          style={{
            ...
            textAlign: "center",
            boxShadow:
              statuses[a.id] === "running" ? `0 0 12px ${a.color}50` : "none",
            transition: "all 0.4s",
            minWidth: 60,
          }}
        >
...
export default function MedTriageApp() {
  ...
  if (!user) return <LoginPage onLoginSuccess={(data) => setUser(data)} />;
  ...
  const simulateAgentProgress = async (data) => {
    for (const agentId of ["extractor", "analyzer", "scorer", "triage"]) {
      ...
      addLog(
        `Agent [${agentId}] ${isDone ? "complete" : "error"}`,
      ...
      <div style={S.shell}>
        ...
        <div style={S.container}>
          ...
          >
            ...
            >
              WHAT MEDICATE IT DOES
            </div>
            {[
              [
                "Symptom Analysis",
                "Describe your symptoms in plain language — our AI reads and structures everything you mention including pain levels, duration, location, and associated symptoms.",
              ],
              [
                "Risk Scoring",
                "A multi-agent pipeline calculates a dynamic risk score from 0 to 100 based on severity, red flags, body systems involved, and onset pattern.",
              ],
              [
                "Disease Prediction",
                "Based on your symptoms, the system identifies the most likely medical conditions with likelihood ratings and clinical reasoning for each.",
              ],
              [
                "Medication Guidance",
                "Alongside each predicted condition, relevant over-the-counter medications and first-aid steps are recommended for your specific symptoms.",
              ],
              [
                "Urgency Triage",
                "You receive a clear urgency verdict — Emergency, Urgent, Semi-Urgent, Non-Urgent, or Self-Care — with a recommended care pathway and timeframe.",
              ],
              [
                "Privacy by Design",
                "Your personal information is stripped before any AI processing. No data is stored, logged, or linked to your identity at any point.",
              ],
            ].map(([title, desc]) => (
              <div
                key={title}
              ...
              >
                ...
                <div>
                  <div
                    style={{
                      color: "#e2e8f0",
                      fontSize: "clamp(14px, 2.5vw, 13px)",
                      fontWeight: 600,
                      marginBottom: 3,
                    }}
                  >
          ...
          >
            ...
            <GraphDiagram
              statuses={Object.fromEntries(AGENTS.map((a) => [a.id, "idle"]))}
            />
            <div
              style={{
                color: "#334155",
        ...
        <div style={S.container}>
          ...
          >
            ...
            >
              ...
              <span
                style={{
                  color: symptoms.length > 50 ? "#22c55e" : "#475569",
                  fontSize: 13,
                  fontFamily: "monospace",
                }}
              >
          ...
          {symptoms.length > 10 && (
            ...
            >
              ...
              >
                {Privacy.stripPII(symptoms).slice(0, 180)}
                {symptoms.length > 180 ? "…" : ""}
              </div>
            </div>
          )}
          ...
          <button
            onClick={runTriage}
            disabled={symptoms.trim().length < 20}
            style={{
              ...S.primaryBtn,
              opacity: symptoms.trim().length < 20 ? 0.4 : 1,
            }}
          >
            Start Assessment
          </button>
      ...
      <div style={S.shell}>
        ...
        <div style={S.container}>
          <div style={{ textAlign: "center", marginBottom: 20 }}>
            <h2 style={{ ...S.title, fontSize: "clamp(15px, 4vw, 18px)" }}>
          ...
          >
            ...
            {!error && (
              <span
                style={{ color: "#334155", animation: "blink 0.8s infinite" }}
              >
                ▋
              </span>
            )}
          ...
          {error && (
            ...
            >
              <p style={{ color: "#fca5a5", fontSize: 15, margin: "0 0 8px" }}>
                ⚠️ {error}
              </p>
              <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>
                Make sure the backend is running at{" "}
                <code style={{ color: "#94a3b8" }}>{API_BASE}</code>
              </p>
              <button
                onClick={reset}
    ...
    return (
      <div style={S.shell}>
        ...
        <div style={S.container}>
          ...
          <GraphDiagram
            statuses={Object.fromEntries(AGENTS.map((a) => [a.id, "done"]))}
          />
          ...
          <div style={S.card}>
            ...
            {(ex.symptoms || []).map((s, i) => (
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {s.location && (
                    <span
                      style={{ color: "#64748b", fontSize: 14, marginLeft: 6 }}
                    >
                      @ {s.location}
                    </span>
                  )}
            ...
            >
              {[
                ...
                [
                  "BODY SYSTEMS",
                  (ex.body_systems || ra.body_systems || []).join(", ") || "—",
                ],
                [
                  "MEDICATIONS TAKEN",
                  ex.current_medications || "None mentioned",
                ],
          ...
          {(ra.red_flags || []).length > 0 && (
            <div style={{ ...S.card, border: "1px solid #7f1d1d" }}>
              <h3 style={{ ...S.cardTitle, color: "#fca5a5" }}>
                Red Flags ({ra.red_flags.length})
              </h3>
          ...
          {(ra.medication_recommendations || []).length > 0 && (
            <div style={{ ...S.card, border: "1px solid #1e3a5f" }}>
              <h3 style={{ ...S.cardTitle, color: "#93c5fd" }}>
                Medication & First-Aid Guidance
              </h3>
              ...
              >
                <p style={{ color: "#475569", fontSize: 13, margin: 0 }}>
                  ⚠️ Always consult a healthcare professional before taking any
                  medication.
                </p>
              </div>
            </div>
          ...
          <div style={S.card}>
            ...
            {(td.self_care_tips || []).length > 0 && (
              <>
                <div
                  style={{ borderTop: "1px solid #1e293b", margin: "10px 0" }}
                />
                <div
                  style={{
                    color: "#334155",
          ...
          >
            ...
            >
              {[
                ...
                [
                  "Identity Linked",
                  pl.session_linked_to_identity ? "✗ Yes" : "✓ No",
                ],
          ...
          >
            ...
            >
              {td.disclaimer ||
                "This AI triage report is informational only. Consult a licensed healthcare professional for diagnosis and treatment."}
            </p>
          </div>
